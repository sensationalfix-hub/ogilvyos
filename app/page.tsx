...
export default function Home() {
  ...
  : <div className="editor-field"><span>Tipo</span><Select value={detail.type} onValueChange={(value) => updateDetailField("type", value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{projectTypeOptions.map((type) => <SelectItem key={type} value={type}>{type}</SelectItem>)}</SelectContent></Select></div>}
          ...
          <div className="editor-field full"><span>Equipo</span><div className="team-multi-select">{team.map((person) => { const selected = detail.people.includes(person.name); return <button type="button" key={person.id} className={selected ? "selected" : ""} onClick={() => { const current = detail.people.filter((name) => name !== "Por asignar"); const nextPeople = selected ? current.filter((name) => name !== person.name) : [...current, person.name]; updateDetailField("people", nextPeople.length ? nextPeople : ["Por asignar"]); }}><span className={`avatar avatar-${person.tone}`}>{person.initials}</span><b>{person.name}</b><small>{person.role}</small>{selected ? <Check /> : <Plus />}</button>; })}</div></div>
